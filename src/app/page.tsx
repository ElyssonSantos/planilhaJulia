...
export default function Dashboard() {
  const { transactions, monthlyLimit, chartType, cards, fixedExpenses } = useFinanceStore();
  ...
  return (
    <div className="flex flex-col min-h-screen pt-6 px-5 pb-32 bg-zinc-50 dark:bg-zinc-950 gap-6 w-full max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
           <button 
             ...
             className={cn(
               "w-12 h-12 glass border-accent/10 rounded-2xl flex items-center justify-center transition-all active:scale-95 relative",
           ...
           <div className="hidden md:flex gap-2">
             <Link href="/settings" className="w-12 h-12 glass border-accent/10 rounded-2xl flex items-center justify-center text-muted-foreground hover:bg-muted/50 transition-all">
                <Settings size={20} />
             </Link>
           </div>
        </div>
        ...
        {showNotifications && (
          <Card className="absolute top-20 right-5 w-72 z-50 glass border-accent/10 rounded-3xl shadow-2xl animate-in slide-in-from-top-4 duration-300 overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        {/* Coluna 1: Saldo e Gráficos */}
        <div className="lg:col-span-8 space-y-6">
          <Card className="bg-zinc-900 border-none rounded-[40px] shadow-2xl relative overflow-hidden group min-h-[220px]">
             ...
             <CardContent className="p-10 space-y-8 relative z-10">
                <div className="space-y-1">
                   <p className="text-[12px] font-black uppercase tracking-[0.4em] text-zinc-500">Saldo Disponível</p>
                ...
                <div className="grid grid-cols-2 gap-8 border-t border-white/5 pt-8">
                   <div className="space-y-1">
                      <div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-widest text-green-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             <Card className="glass border-accent/10 rounded-[40px] overflow-hidden shadow-2xl">
                ...
                <CardContent className="h-[280px] w-full mt-4">
             ...
             <div className="space-y-6">
                {monthlyLimit > 0 && (
                  <Card className={cn(
                    "rounded-[40px] border-none shadow-xl transition-all h-full max-h-[160px]",
                    isOverLimit ? "bg-red-500 text-white" : limitReached > 80 ? "bg-yellow-400 text-yellow-950" : "bg-white dark:bg-zinc-900"
                  )}>
                    <CardContent className="p-8 flex flex-col justify-between h-full">
                       <div className="flex items-center justify-between">
                          <span className="text-[12px] font-black uppercase tracking-widest">Limite Mensal</span>
                          {isOverLimit ? <AlertTriangle size={20} /> : <Target size={20} />}
                       </div>
                       <div className="space-y-2">
                          <div className="flex justify-between items-end">
                            <span className="text-2xl font-black tracking-tighter">{limitReached.toFixed(0)}%</span>
                            <span className="text-[10px] font-bold opacity-60">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(totalExpense)} / {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(monthlyLimit)}</span>
                          </div>
                          <div className="h-3 w-full rounded-full bg-black/10 overflow-hidden">
                             <div className="h-full bg-current rounded-full transition-all duration-1000" style={{ width: `${Math.min(limitReached, 100)}%` }} />
                          </div>
                       </div>
                    </CardContent>
                  </Card>
                ...
                <Card className="glass border-accent/10 rounded-[40px] overflow-hidden shadow-2xl p-8 flex flex-col justify-center">
                   <div className="flex items-center justify-between mb-4">
                      <p className="text-[11px] font-black uppercase tracking-widest text-muted-foreground opacity-60">Status do Banco</p>
                      <Sparkles size={18} className="text-green-600" />
                   </div>
                   <p className="text-xl font-black tracking-tight leading-snug">
                     {balance > 0 ? "Você está mandando bem, Julia! Tudo sob controle. 🐷💎" : "Hora de economizar um pouco mais este mês! 🛡️💰"}
                   </p>
                </Card>
             </div>
          </div>
        ...
        {/* Coluna 2: Cartões, Fixos e Atividade */}
        <div className="lg:col-span-4 space-y-6">
           {/* Atalhos Rápidos */}
           <div className="grid grid-cols-2 gap-4">
              <Link href="/cards" className="glass border-accent/10 p-5 rounded-[30px] flex flex-col items-center gap-2 hover:bg-green-600 hover:text-white transition-all group">
                 <CreditCard size={24} className="group-hover:scale-110 transition-transform" />
                 <span className="text-[10px] font-black uppercase tracking-widest text-center">{cards.length} Cartões</span>
              </Link>
              <Link href="/fixed-expenses" className="glass border-accent/10 p-5 rounded-[30px] flex flex-col items-center gap-2 hover:bg-green-600 hover:text-white transition-all group">
                 <CalendarClock size={24} className="group-hover:scale-110 transition-transform" />
                 <span className="text-[10px] font-black uppercase tracking-widest text-center">{fixedExpenses.length} Fixas</span>
              </Link>
           </div>

           <Card className="glass border-accent/10 rounded-[40px] overflow-hidden flex flex-col">
              <CardHeader className="pb-4">
                 <div className="flex items-center justify-between">
                    <h3 className="text-[11px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                       <History size={16} /> Atividade
                    </h3>
                    <Link href="/extrato" className="text-[10px] font-black uppercase text-green-600 hover:underline">Ver tudo</Link>
                 </div>
              </CardHeader>
              <CardContent className="space-y-4 px-6 pb-6">
                 {recentTransactions.map((t) => (
                   <div key={t.id} className="flex items-center justify-between group transition-all">
                      <div className="flex items-center gap-3">
                         <div className={cn(
                         ...
                         <div>
                            <p className="text-[12px] font-black text-foreground leading-none mb-1 uppercase truncate max-w-[100px]">{t.description || t.category}</p>
                            <p className="text-[8px] font-bold uppercase text-muted-foreground/40">{format(new Date(t.date), "dd/MM")}</p>
                         </div>
                      </div>
                      <div className={cn("text-xs font-black tracking-tighter", t.type === 'expense' ? "text-red-500" : "text-green-600")}>
                         {t.type === 'expense' ? '-' : '+'}
           ...
           <Link href="/add" className="w-full h-16 bg-green-600 hover:bg-green-700 text-white rounded-[24px] flex items-center justify-center gap-3 shadow-xl shadow-green-600/30 transition-all active:scale-95 group">
              <Plus size={24} className="group-hover:rotate-90 transition-transform duration-300" />
              <span className="text-sm font-black uppercase tracking-widest">Novo Lançamento</span>
           </Link>
        </div>
      </div>
    </div>
